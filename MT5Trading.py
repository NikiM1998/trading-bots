...
class MT5:
   ...
   # Add dictionaries to store the highest and lowest prices for each open trade
   highest_prices = {}
   lowest_prices = {}
   ...
   def orders(symbol, lot, buy=True, id_position=None):
       ...
       pip_value = (1.0 / symbol_info.session_close) * lot
       ...
       if id_position == None:
           ...
           # Store the opening price as the highest and lowest price for this trade
           MT5.highest_prices[result.order] = price
           MT5.lowest_prices[result.order] = price
       ...
       # **************************** Close a trade *****************************
           ...
           # Remove this trade from highest_prices and lowest_prices
           if id_position in MT5.highest_prices:
               del MT5.highest_prices[id_position]
           if id_position in MT5.lowest_prices:
               del MT5.lowest_prices[id_position]
   ...
   def close_all_night():
    ...
    for i in range(len(result)):
        ...
        trade_id = row["ticket"][0]
        # if trade_id not in MT5.lowest_prices:
            #print(f"Trade {trade_id} does not exist in lowest_prices")
            
        # if trade_id not in MT5.highest_prices:
            #print(f"Trade {trade_id} does not exist in highest_prices")
        ...
        if row["position"][0]==0:
            res = MT5.orders(row["symbol"][0], row["volume"][0], buy=True, id_position=trade_id)

        else:
            # Calculate drawdown
            res = MT5.orders(row["symbol"][0], row["volume"][0], buy=False, id_position=trade_id)

        # Remove this trade from highest_prices and lowest_prices
        if trade_id in MT5.highest_prices:
            del MT5.highest_prices[trade_id]
        if trade_id in MT5.lowest_prices:
            del MT5.lowest_prices[trade_id]
